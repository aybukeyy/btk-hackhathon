...
# Session and topic prompt history
topic_prompts = {}
chat_sessions = {}
...
def chat_message():
    ...
    if topic_id not in chat_sessions:
        ...
        if isinstance(initial_prompt_text, dict):
            # Bu yapı artık kullanılmıyor ama uyumluluk için kalabilir.
            # Yeni promptlar tek bir metin olarak gelecek.
    ...
    try:
        if not user_message:
            if len(chat.history) == 1:
                ...
                # Tetikleyici mesajı Türkçe yapıyoruz.
...
def get_all_quiz_lists():
    """
    'lists' klasörü içindeki 'main_lists' ve 'user_lists' alt klasörlerini
    doğru bir şekilde tarar ve tüm .json dosyalarını birleştirip döner.
    """
    all_lists = set()
